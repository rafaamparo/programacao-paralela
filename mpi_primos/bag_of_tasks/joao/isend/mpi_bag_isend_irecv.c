    ...
    if (meu_ranque == raiz) {
        ...
        for (dest = 1; dest < num_procs; dest++) {
            if (inicio < n) {
                ...
                MPI_Isend(&valor_inicio, 1, MPI_INT, dest, tag, MPI_COMM_WORLD, &request);
                // Espera o envio completar antes de reutilizar a variável
                MPI_Wait(&request, &status_isend);
                inicio += TAMANHO;
            } else {
        ...
        while (stop < num_procs - 1) {
            ...
            MPI_Irecv(&cont, 1, MPI_INT, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &recv_request);
        ...
        while (1) {
            ...
            // Espera o envio completar antes de continuar
            MPI_Wait(&request, &status_isend);
        }
        t1 = MPI_Wtime();
    }
