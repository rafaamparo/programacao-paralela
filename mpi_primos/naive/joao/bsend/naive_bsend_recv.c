...
int main(int argc, char *argv[]) {
	...
	if(num_procs > 1) {
		if (meu_ranque == 0) {
			...
			// Envia a contagem local para o processo 0 usando MPI_Bsend
			MPI_Bsend(&cont, 1, MPI_INT, 0, 0, MPI_COMM_WORLD);
			// Não é necessário MPI_Wait com MPI_Bsend
		}
	} else {
		total = cont;
	}
	...
	// Liberar o buffer antes de finalizar o MPI
	void* buffer_addr;
	int buffer_size_out;
	MPI_Buffer_detach(&buffer_addr, &buffer_size_out);
	free(buffer_addr);
