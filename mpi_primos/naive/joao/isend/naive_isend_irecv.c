...
int main(int argc, char *argv[]) {
	...
	if(num_procs > 1) {
		// Usando MPI_Isend no lugar de MPI_Send
		if (meu_ranque == 0) {
			total = cont; // Inicializa com os primos encontrados pelo processo 0
			int cont_outros;
			MPI_Request requests[num_procs-1]; // Array para armazenar os identificadores de solicitação
			
			// Inicia todas as recepções não-bloqueantes
			for (int origem = 1; origem < num_procs; origem++) {
				MPI_Irecv(&cont_outros, 1, MPI_INT, origem, 0, MPI_COMM_WORLD, &requests[origem-1]);
			}
			
			// Espera cada recebimento ser concluído e atualiza o total
			for (int origem = 1; origem < num_procs; origem++) {
				MPI_Wait(&requests[origem-1], MPI_STATUS_IGNORE);
				total += cont_outros;
			}
		} else {
			// Envia a contagem local para o processo 0 usando MPI_Isend
			MPI_Request request;
			MPI_Isend(&cont, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, &request);
			// Garantir que o envio seja concluído antes de continuar
