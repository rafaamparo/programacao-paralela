...
int main(int argc, char *argv[]) {
	...
	if(num_procs > 1) {
		// Sincronização para garantir que o processo 0 tenha postado o MPI_Recv
		...
		if (meu_ranque == 0) {
			...
			// Recebe a contagem de cada processo
			for (int origem = 1; origem < num_procs; origem++) {
				// Garantimos que este MPI_Recv está postado antes do MPI_Rsend ser chamado
				MPI_Recv(&cont_outros, 1, MPI_INT, origem, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
				total += cont_outros;
			}
		} else {
			// Espera um pouco para garantir que o processo 0 já postou todos os MPI_Recv
			// MPI_Barrier(MPI_COMM_WORLD);
			
			// Usando MPI_Rsend que exige que o receptor já tenha postado o MPI_Recv
