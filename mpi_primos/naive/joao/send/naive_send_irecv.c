...
int main(int argc, char *argv[]) {
	...
	if(num_procs > 1) {
		// Substituindo MPI_Reduce por MPI_Send e MPI_Irecv
		if (meu_ranque == 0) {
			...
			int *recvcounts = (int*)malloc((num_procs-1) * sizeof(int));
			
			// Inicia recebimentos não-bloqueantes de cada processo
			for (int origem = 1; origem < num_procs; origem++) {
				MPI_Irecv(&recvcounts[origem-1], 1, MPI_INT, origem, 0, MPI_COMM_WORLD, &requests[origem-1]);
			}
			
			// Espera todos os recebimentos completarem
			MPI_Waitall(num_procs-1, requests, MPI_STATUSES_IGNORE);
			
			// Soma as contagens recebidas
			for (int origem = 1; origem < num_procs; origem++) {
				total += recvcounts[origem-1];
			}
			...
			// Envia a contagem local para o processo 0
